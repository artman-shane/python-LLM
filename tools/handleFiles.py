...
class HandleFiles:
    ...
    def read_xlsx(self, file_path, headers=0):
        ...
        try:
            ...
            if os.getenv('DEBUG'): print(f"Processing...")
            dfs = pd.read_excel(file_path, sheet_name=None)
            if os.getenv('DEBUG'): print(f"Number of sheets: {len(dfs)}")
            for sheet_name, df in dfs.items():
    ...
    def _create_xlsx(self, file_path):
        try:
            if os.getenv('DEBUG'): print(f"Creating file: {file_path}")
            if os.getenv('DEBUG'): print(f"Creating empty data frame...")
            # Create a DataFrame with no data.
            df = pd.DataFrame()
            if os.getenv('DEBUG'): print(f"Writing to Excel file...")
            # Write the DataFrame to an Excel file
            df.to_excel(file_path, index=False)
            if os.getenv('DEBUG'): print(f"File created: {file_path}")
            return True
        except Exception as e:
            raise ValueError(f"\nAn error occurred: {e}")
    ...
    def clean_json(self, json_str):
        # Clean JSON
        if os.getenv("DEBUG"): print(f"Is JSON dirty?")
        # Use regular expression to find the first '{' and the last '}'
        if os.getenv('DEBUG_DATA_OUTPUT'): print(f"\n***** BEGIN DIRTY *****\n{json_str}\n***** END DIRTY *****\n")
        match = re.search(r'[\{,\[].*[\},\]]', json_str, re.DOTALL)
        if match:
            if os.getenv("DEBUG"): print(f"Match Found. Cleaning JSON...")
            if os.getenv('DEBUG_DATA_OUTPUT'): print(f"JSON cleaned:\n*****BEGIN CLEANED*****\n{match.group(0)}\n***** END CLENAED *****\n")
    ...
    def write_xlsx(self, file_path, data_dict):
        ...
        try:
            ...
            if os.getenv('DEBUG'): print(f"JSON Clean")
            if os.getenv('DEBUG'): print(f"Checking if file exists...")
            if not os.path.exists(file_path):
                if os.getenv('DEBUG'): print(f"File not found: {file_path} must be created...")
                filename = os.path.basename(file_path)
                if os.getenv('DEBUG'): print(f"Filename full path to create: {filename}")
                self._create_xlsx(os.path.join(self.folder_path, filename))
                if os.getenv('DEBUG'): print(f"Successfully created file: {file_path}")
            else:
                if os.getenv('DEBUG'): print(f"File found: {file_path}")
            if os.getenv('DEBUG_DATA_OUTPUT'): print(f"Data to write: *****\n{data_dict}\n*****")
            if os.getenv('DEBUG'): print(f"Converting to DataFrame...")
            data = json.loads(data_dict)
            if os.getenv("DEBUG"): print(f"VarType:\ndata_dict: {type(data_dict)}\ndata: {type(data)}")
            if os.getenv('DEBUG_DATA_OUTPUT'): print(f"Data as JSON: {data}")

            # Convert the JSON object to a DataFrame
            # df = pd.read_json(json.loads(data_dict))

            if os.getenv('DEBUG'): print(f"Creating Blank DataFrame array...")
            combined_df = pd.DataFrame()
            if os.getenv('DEBUG'): print(f"Checking number of sheets...")
            if len(data.keys()) > 1:
                if os.getenv('DEBUG'): print(f"More than one sheet found...")
                for sheet_name in data.keys():
                    if os.getenv('DEBUG'): print(f"Sheet Name: {sheet_name}")
                    df = pd.DataFrame(data[sheet_name])
                    if os.getenv('DEBUG'): print(f"Inserting sheet name...")
                    df.insert(0, 'Sheet Name', sheet_name)
                    if os.getenv('DEBUG'): print(f"Combining DataFrames...")
                    combined_df = pd.concat([combined_df, df], ignore_index=True)
            else:
                if os.getenv('DEBUG'): print(f"Only one sheet found...")
                if os.getenv('DEBUG'): print(f"Creating DataFrame...")
            ...
            if os.getenv('DEBUG'): print(f"Converting df to logic excel file...")
            if os.getenv('DEBUG_DATA_OUTPUT'): print(f"\n***** BEGIN Data to write *****\n{combined_df}\n***** END Data to write *****\n")
            # Write the DataFrame to an Excel file
