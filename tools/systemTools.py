...
class SystemTools:
    ...
    def getFlatJson(self, _questions):
        if self.str_to_bool(os.getenv('DEBUG_FUNCTION_NAME')): print(f"***** BEGIN func getFlatJson *****")
        # Test if object is json
        try:
            # Check if there is a parent key. Could be a sheet or page key.
            if self.str_to_bool(os.getenv('DEBUG')): print(f"Checking if questions is a dictionary")
            if isinstance(_questions, dict):
                if self.str_to_bool(os.getenv('DEBUG')): print(f"Questions is a dictionary. Collapsing...")
                collapsedQuestions = []
        ...
        if self.str_to_bool(os.getenv('DEBUG_FUNCTION_NAME')): print(f"***** END func getFlatJson *****")
